...
- TypeSafe: start at https://docs.typesafe.ai/llms.txt; the wire contract is https://docs.typesafe.ai/api.md. Load the `typesafe:typesafe-ai` skill with the Skill tool first: it carries the current guidance. The OpenAPI document (https://api.typesafe.ai/openapi.json) is vendored at `crates/judgment/tests/fixtures/typesafe-openapi.json` and checked by `crates/judgment/tests/contract.rs` (the crate's requests, Fakes and recordings, and the pinned places where the crate is stricter or looser than the schema) and `tests/typesafe_contract.rs` (signalman's triage requests and TypeSafe mocks). Run `cargo test -p judgment --test openapi_drift -- --ignored` to see whether the copy is stale; never accept a hand edit of the fixture.
...
7. Tests: a changed shape has a matching change in `crates/judgment/tests/client.rs`, `crates/judgment/tests/contract.rs`, `tests/typesafe_contract.rs`, `tests/incidentio.rs`, `tests/webhook_server.rs`, `tests/mcp_server.rs` or `tests/mcp_http.rs`. A new TypeSafe request or response shape is added to the contract test's question set or fixtures, and a new deliberate difference from the vendored schema is pinned there at its own path.

...
- It has run once against a live TypeSafe model (`jev-1.13.0`, the three example alerts, every decision as labelled) a live Backstage catalog and, read-only, a live incident.io account (`whoami`, incidents, alerts, alert notes); it has not yet written a tag, an attachment or a note to a live alert, nor reached TechDocs or Notifications. Every other wire shape is asserted against the published documentation and OpenAPI specification, not observed traffic. See [the roadmap](docs/roadmap.md).

...
- Keep unknown response fields, or at least do not fight them. Laya's `routing`, `answer_confidence` and `action`, and other servers' extras, are dropped by serde today, which is correct for typing; a raw `extra` map on `Response` would let an operator log which checkpoint answered without a second type change later. Low priority until a consumer asks.

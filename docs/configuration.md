...
| `OTEL_EXPORTER_OTLP_HEADERS` | the OTLP exporter, when the collector needs a token or key ([Observability](observability.md#configuration)) | `key=value,key2=value2`, for example `authorization=Bearer …`; read by the exporter itself, never by `src/config.rs`; the per-signal `OTEL_EXPORTER_OTLP_TRACES_HEADERS` and `OTEL_EXPORTER_OTLP_METRICS_HEADERS` work the same way |

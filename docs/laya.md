...
The [configuration layer](configuration.md) already makes the model endpoint a setting. `examples/laya/serve_laya.py` is a fifty-line HTTP shim that loads a checkpoint with the `laya` package and serves `POST /v1/systemone` and `GET /v1/models`:

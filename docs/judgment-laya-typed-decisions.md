...
| `a_structured_score_level_is_echoed` | An object level, an array level and a string level are sent, the body fetched directly rather than through the client, the legend printed exactly as the server echoed it, and `Response::verify` passes on it | `verify` accepts a structured level echoed either as itself or as its compact JSON; the test records which of the two a server actually does, so the rule can be tightened to it. Added with judgment 0.2 |
...
- The top-level `routing` field arrived in `Response::extra`.
- The server sends no `x-typesafe-request-id` header, so the printed request id was `None`. This is an observation from the printed output; no test asserts it.

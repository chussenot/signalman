...
signalman writes alert tags and alert-to-incident attachments, or forwards enriched alerts to an HTTP alert source. It does not create, edit or resolve incidents.

## Consequences

- One routing model. Operators change escalation in incident.io, not in signalman's code.
...
- Anything signalman wants incident.io to do must be expressible as an alert route condition. If that proves insufficient, this decision is revisited rather than worked around.
